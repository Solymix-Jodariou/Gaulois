...
async def build_leaderboard_ffa_embed(guild, page: int, page_size: int):
    ...
    header = f"{'#':<3} {'JOUEUR':<{name_width}} {'DISCORD':<{discord_width}} {'SCORE':>6} {'W/L':>9} {'G':>4}"
    sep = "-" * (name_width + discord_width + 34)
    table = [header, sep]
    for i, p in enumerate(page_items, start + 1):
        ...
        games = f"{p['games']}"
        table.append(
            f"{i:<3} {name:<{name_width}} {discord_name:<{discord_width}} {score:>6} {wl:>9} {games:>4}"
        )
