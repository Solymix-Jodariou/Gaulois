...
from api_openfront import *  # noqa: F403
from embeds_discord import *  # noqa: F403
...
async def build_leaderboard_embed(guild, page: int, page_size: int):
    ...
    if not top:
        return None
    return build_leaderboard_embed_from_data(guild, page, page_size, top, last_updated)
...
async def build_leaderboard_ffa_embed(guild, page: int, page_size: int):
    ...
    if not top:
            return None
    return await build_leaderboard_ffa_embed_from_data(guild, page, page_size, top, last_updated)
...
async def build_leaderboard_1v1_embed(guild, page: int, page_size: int):
    ...
    if not top:
        return None
    return build_leaderboard_1v1_embed_from_data(guild, page, page_size, top, last_updated)
...
async def build_leaderboard_1v1_gal_embed(guild):
    ...
    if not gal_items:
        return None
    return build_leaderboard_1v1_gal_embed_from_data(guild, gal_items, last_updated)
...
async def build_ofm_board_embed(guild: discord.Guild):
    ...
    return build_ofm_board_embed_from_data(guild, rows, team_name)
    if not rows:
...
async def build_ofm_admin_panel_embed(guild: discord.Guild):
    team_name = await get_ofm_team_name(guild.id) or DEFAULT_OFM_TEAM_NAME
    return build_ofm_admin_panel_embed_from_data(team_name)
    embed = discord.Embed(
...
async def update_leaderboard_message_for_guild(guild: discord.Guild):
    record = await get_leaderboard_message(guild.id)
    if not record:
...
async def resync_leaderboards(guild: discord.Guild):
    ...
    main_message = await update_leaderboard_message_for_guild(guild)
    ffa_recovered = None
...
async def resyncleaderboards(interaction: discord.Interaction):
    ...
    try:
        ...
        main_msg = result.get("main_message") or {}
        main_msg_text = "OK" if main_msg.get("updated") else f"KO ({main_msg.get('error')})"
        await interaction.followup.send(
            ...
            f"Maj message principal: {main_msg_text}",
            ephemeral=True,
        )
    except Exception as exc:
...
async def botstatus(interaction: discord.Interaction):
    ...
    embed.add_field(name="Leaderboard 1v1", value=lb_1v1_text, inline=False)
    embed.add_field(name="Leaderboard 1v1 GAL", value=lb_1v1_text, inline=False)
